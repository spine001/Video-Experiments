// index.js
'use strict';

// Loading dependencies & initializing express
const os = require('os') // for operating system-related utility methods and properties
const express_s = require('express')
const express = require('express')
const http = require('http') // for creating http server
const path = require('path')
const morgan = require('morgan')
/**********************************/
const https = require('https'); // for creating https server
const fs = require("mz/fs"); // for input, validation, reading SSL certificates
const pem = require('pem'); // for temporary creation of self signed SSL certificates
const session = require('cookie-session');
/**********************************/

/**********************************/
// Get a self signed SSL for the first sign up to get the actual certificate from letsencryp.org
const { key, cert } = async function () {
    const { key, cert } = await (async () => {
        try {
            const certdir = (await fs.readdir("/etc/letsencrypt/live"))[0];

            return {
                key: await fs.readFile(`/etc/letsencrypt/live/${certdir}/privkey.pem`),
                cert: await fs.readFile(`/etc/letsencrypt/live/${certdir}/fullchain.pem`)
            }
        } catch (e) {
            return new Promise((res, rej) => {
                pem.createCertificate({ days: 1, selfSigned: true }, function (err, keys) {
                    if (err) {
                        console.log("unexpected error:")
                        console.log(err)
                        rej();
                    } else {
                        res({ key: keys.serviceKey, cert: keys.certificate });
                    }
                });
            });
        }
    })()
    return (key, cert)
}();

/**********************************/

// For signalling in WebRTC
const socketIO = require('socket.io')

const app = express()
const app_s = express_s()

/**********************************/
// Prevent theft of cookies from http
// app.use(
//   session({
//     secret: "pantaleon meaba pantalon witch",
//     httpOnly: true,  // Don't let browser javascript access cookies.
//     secure: true, // Only use cookies over https.
//   })
// )
/**********************************/

// log httpactivity 
app.use(morgan('dev'))
app_s.use(morgan('dev'))

/**********************************/
// Redirect to https all http requests
app.get('*', (req, res) => {
    if (!req.secure) { // only redirect http resquests
        res.redirect('https://' + req.headers.host + req.url)
        console.log("redirected to:")
        console.log(`${req.headers.host}${req.url}`)
    };
    // Or, if you don't want to automatically detect the domain name from the request header, you can hard code it:
    // res.redirect('https://example.com' + req.url);
})
/**********************************/

// Define the folder which contains the CSS and JS for the frontend
// Configure app. express.static will also serve the favicon.ico file located in /public, thanks to a link placed in the html

app_s.use('/public', express.static(path.resolve(__dirname, 'public')))

//Define a route 
app_s.get("/", function (req, res) {
    // Render a view (located in the directory views/) on this route
    res.render("index.ejs")
});

/**********************************/
// For https:

const httpsServer = https.createServer({ key, cert }, app_s)
httpsServer.listen(443)
/**********************************/

// For http:
// Initialize http server and associate it with express
const server = http.createServer(app)

// Ports on which server should listen - 8000 or the one provided by the environment
server.listen(process.env.PORT || 80)

// Initialize socket.io for http
// const io = socketIO(server)

/**********************************/
// for https change to:
const io = socketIO(httpsServer)
/**********************************/

// Implementing Socket.io
// connection is a synonym of reserved event connect
// connection event is fired as soon as a client connects to this socket.
io.sockets.on('connection', function (socket) {

    // Convenience function to log server messages on the client.
    // Arguments is an array like object which contains all the arguments of log(). 
    // To push all the arguments of log() in array, we have to use apply().
    function log() {
        var array = ['Message from server:'];
        array.push.apply(array, arguments);
        socket.emit('log', array);
    }


    //Defining Server behavious on Socket Events
    socket.on('message', function (message, room) {
        log('Client said: ', message);
        //server should send the receive only in room
        socket.in(room).emit('message', message, room);
    });

    //Event for joining/creating room
    socket.on('create or join', function (room) {
        log('Received request to create or join room ' + room);

        //Finding clients in the current room
        var clientsInRoom = io.sockets.adapter.rooms[room];
        var numClients = clientsInRoom ? Object.keys(clientsInRoom.sockets).length : 0;
        log('Room ' + room + ' now has ' + numClients + ' client(s)');

        //If no client is in the room, create a room and add the current client
        if (numClients === 0) {
            socket.join(room);
            log('Client ID ' + socket.id + ' created room ' + room);
            socket.emit('created', room, socket.id);
        }

        //If one client is already in the room, add this client in the room
        else if (numClients === 1) {
            log('Client ID ' + socket.id + ' joined room ' + room);
            io.sockets.in(room).emit('join', room);
            socket.join(room);
            socket.emit('joined', room, socket.id);
            io.sockets.in(room).emit('ready');
        }

        //If two clients are already present in the room, do not add the current client in the room
        else { // max two clients
            socket.emit('full', room);
        }
    });

    //Utility event 
    socket.on('ipaddr', function () {
        var ifaces = os.networkInterfaces();
        for (var dev in ifaces) {
            ifaces[dev].forEach(function (details) {
                if (details.family === 'IPv4' && details.address !== '127.0.0.1') {
                    socket.emit('ipaddr', details.address);
                }
            });
        }
    });

    //Event for notifying other clients when a client leaves the room
    socket.on('bye', function () {
        console.log('received bye');
    });

});